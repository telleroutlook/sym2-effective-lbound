...
**Theorem** (Friedlander-Iwaniec, Proposition 3.2, "Primes in arithmetic
progressions to large modulus", Cambridge University Press):
...
[THM, standard normalization for sym^2 of weight-k form with k=12]
...
1. **Explicit constant C:** Friedlander-Iwaniec proves C(epsilon) exists
   ...
   max |S(X)|/X^{0.5} = 0.259 for X in [100, 20000] is discovery-tier.
...
## §5. Connection to L(1) (corrected truncation)
...
**Note:** This uses the correct Abel summation truncation, NOT the
incorrect Cesàro平均 formula previously stated.

## §6. References

- Friedlander-Iwaniec (1998), "Primes in arithmetic progressions to large
  modulus", Acta Math. — Proposition 3.2, degree-3 partial sum bound.
- Gelbart-Jacquet (1978), "A note on the symmetric square L-function" — GL_3
  structure, entireness.
