...
GOAL: Compute the N=10^8 Cesaro approximation and display conditional lower
bounds that would follow from a future proof of C_GL3.

CONDITIONAL STRUCTURE:
  L(1) = L_ces(N, 1) + [L(1) - L_ces(N, 1)]
  |L(1) - L_ces(N, 1)| <= 4 * C_GL3 / N^{1/3}     [Abel summation]

  With C_GL3 <= 1.476 (a conditional sketch) and N=10^8:
    error <= 4 * 1.476 / (10^8)^{1/3} = 0.01272

  With C_GL3 <= Q^{1/3} = 6.93 (standard GL3 Voronoi):
    error <= 4 * 6.93 / (10^8)^{1/3} = 0.05975

OUTPUT: L_ces(10^8, 1), and conditional bounds under candidate C_GL3 assumptions.
The float sieve output is discovery-tier and is not a checker certificate.
"""
import sys
...
print("=== L_ces(N=10^8, s=1) computation ===", flush=True)
...
C_GL3_arb  = 1.476    # conditional L1-route sketch
C_GL3_Q13  = 6.930    # Q_GL3^{1/3} (standard GL3 Voronoi)
C_GL3_15   = 15.0     # very conservative

print("\n=== Conditional lower-bound sketches for L(1, sym^2 Delta) ===")
print(f"  L_ces(10^8, 1) = {L_ces_1:.8f}")
print()
for label, C in [("C_GL3 <= 1.476 (conditional L1 sketch)", C_GL3_arb),
    ...
    print(f"    L(1) in [{lb:.5f}, {ub:.5f}] if the assumed C_GL3 is proved; positivity: {'YES' if lb > 0 else 'NO'})")
    print()
