...
- **偏和实验扩展（2026-08-16，`discovery/_fast_tau_sieve.py`，N=1000000）**：
  ...
  **Arb 认证 GL3 Bessel 范数（2026-08-17，`discovery/_cgl3_arb_cert.py`，200-bit 精度）**：
  - K_nu(0+) = C_0 = -0.5143（Arb 认证至 59 位小数）
  - 大 y 系数 C_1 = 0.00261（Arb 认证至 61 位）
  - **||K_nu||_1 ≤ 0.184（Arb 认证）**（含尾部修正：y<0.01 贡献 0.005，y>100 贡献 0.001）
  **三条件 C_GL3 界（均远低于阈值，`discovery/_voronoi_proof_sketch.py`）**：
  | 路线 | 公式 | C_GL3 上界 | 与 7.488 的裕量 |
  |---|---|---|---|
  | L1 + ζ(3/2) | 2×||K||_1×ζ(3/2) | **0.961** | **7.8×** |
  | L1 + ζ(4/3) | 2×||K||_1×ζ(4/3) | **1.325** | **5.7×** |
