...
- **正确的 [OBL E-2] 路径（待实现）**：
  使用**标准 GL₃ AFE（含主和与对偶和）**直接计算 L(1/2+it_k)，再数值积分求 J：
  1. 对每个积分节点 t_k：L(1/2+it_k) = Σ_{n≤N_t} a(n)/n^{1/2+it_k} × V+(n/X_t) + ε × Σ_{n≤N_t} a(n)/n^{1/2-it_k} × V-(n/X_t) + O(exp(-c N_t^{1/3}))，其中 N_t ~ Q^{1/3}(1+|t|)^{2/3} ≈ 10–20 项（无需 Gaussian 正则化；使用标准 GL₃ AFE 光滑截断）
  2. J_quad = (Δt/2π) × Σ_k Re[L_AFE(1/2+it_k) × amp1(t_k)]，积分区间 |t| ≤ 5（amp1 的高斯截断给出指数小尾项）
  3. 用 Arb 区间算术认证 V+、V-、amp1 及各误差项
  此方法每次 t 只需 ~20 项，总计 200 个积分节点 × 20 项 = 4000 次求值，**计算可行**。
