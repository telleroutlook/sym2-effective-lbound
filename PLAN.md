...
- **偏和实验扩展（2026-08-16，`discovery/_fast_tau_sieve.py`，N=1000000）**：
  ...
  **N=10^6 Cesaro 零点扫描**（`discovery/_growth_and_minL.py`，t 步长 0.005）：
  ...
  - **N=10^8 扫描目标（`discovery/_n10m8_dc_scan.py`，运行中）**：
    - 预期 tail_factor ≈ 0.0528；**阈值 ≈ 7.44 > Q_GL3^{1/3} = 6.93**
    - 若 GL3 理论给出 C_GL3 ≤ Q_GL3^{1/3}（最弱 Voronoi 界），则认证成立
    - Q_GL3^{1/3} = (Π|ν_i-ν_j|)^{1/3} = 332.75^{1/3} = 6.93 是最容易从 Miller-Schmid 提取的常数
