...
- Tauberian 渐近：∑_{n≤N} τ(n)²/n^{11} / N → L(1,sym²Δ)/ζ(2)，故
  **L(1, sym²Δ) = ζ(2) × 0.3839 ≈ 0.631**（N=5000 探索层估计）。
- 与 Dirichlet 级数验证一致：∑_{n≤3000} a_{sym²}(n)/n^{1.01} ≈ 0.634 ≈ L(1)。
- 与 Euler 乘积（25 个素数）一致：∏_{p≤97} L_p(1) ≈ 0.641 ≈ L(1)。
  （Euler 乘积在 s=1 处**条件收敛**；因 (c_p²-1)/p 的 Sato-Tate 均值为 0，
  但简单尾部界 ∑_{p>P} 3/p 发散，故无法通过此方式认证。）
