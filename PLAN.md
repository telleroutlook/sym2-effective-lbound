...
### 2026-08-18 状态审计结论（已更新 2026-08-20）

- **F-3 [THM]**：L(1, sym²Δ) ∈ [0.63179293, 0.63179298] 已通过 Arb 认证
  （宽度 4.6×10⁻⁸），证书在 `witness/single_point_certificate.json`。
- **M-3 [THM for Δ]**：零点自由区域 σ∈[0.6,1], |t|≤20 已通过重叠圆盘论证证明
  （205 网格点，160 cell center 全覆盖），证明在 `proof/04b-zero-free-region.md`。
- **J 认证 [THM]**：J = S1 − L(1) ∈ [−0.083495, −0.083487] 已认证
  （宽度 7×10⁻⁶），证书在 `witness/j_certificate.json`。
...
> **2026-08-18 状态修正（已更新 2026-08-20）**：原先“单实例 AFE 可完全绕开 M-3”的判断过于乐观。对 sym²Δ，主和 S1 可以用 Arb 认证，但对偶/围道项 J 的截断误差需要实例级零点自由区域。现已完成：零点自由区域 σ∈[0.6,1], |t|≤20 已证明；J 通过 J = S1 − L(1) 路线认证（不依赖 GL₃ Voronoi）。统一族群界 E-1 仍走 c_eff / log p 路线。
