...
- **偏和实验扩展（2026-08-16，`discovery/_fast_tau_sieve.py`，N=1000000）**：
  ...
  **GL₃ Voronoi 路径可行条件**：若 |S(X)| ≤ C_GL3 × X^{2/3}（GL₃ Voronoi 界），则
  N=10^6 时 Cesaro 误差 = 3 × C_GL3 × (10^6)^{2/3−0.9} = 3 × C_GL3 / 1715。
  要求：C_GL3 < 1715 × 0.449 / 3 ≈ 256。即 **GL₃ Voronoi 显式常数 C_GL3 ≤ 256 就足够**。
  
  **理论障碍（单一，具体化）**：需要下列任意一项：
  (a) 证明 |S(X)| ≤ 0.445 × X^{0.37} 对所有 X（实际增长率，如果这是真实规律）
  (b) 证明 |S(X)| ≤ C × X^{2/3}（GL₃ Voronoi）并给出显式 C ≤ 256
  (c) 有效 GL₃ PNT：|S(X)| = O(X / exp(c√log X))（任意小次幂增长，c 可显式）
