...
- **偏和实验扩展（2026-08-16，`discovery/_fast_tau_sieve.py`，N=500000）**：
  快速重现：log-导数递推 n×τ(n)=-24Σσ₁(k)τ(n-k) + 乘法筛（GL₃ Hecke 递推）。耗时：
  N=500000 计算 τ(n) 需 35s，a_{sym²}(n) 需 0.5s。结果：
  
  | N_max | max|S(X)| | 位置 X | Cesaro 误差界 (σ=0.9) |
  |-------|-----------|--------|----------------------|
  | 10000 | 13.3 | 7925 | 0.0100 |
  | 20000 | 15.4 | 18806 | 0.0062 |
  | 100000 | 26.1 | 94048 | 0.0025 |
  | 500000 | **52.13** | **224786（高原！）** | **0.0012** |
  
  **关键：max|S(X)| 在 X=224786 达到 52.13 后出现高原——对 X ∈ [300000, 500000] 不再增长。**
  
  **认证具体化为单一猜想**：若能证明 |S(X)| ≤ 53 对所有 X ≥ 1 成立（对 X ≤ 500000 已数值验证），则：
  - Cesaro 误差界 ≈ 3×53/N^{0.9}，在 N=500000 时为 0.0012
  - min|L_ces(0.9+7.075i)| ≈ 0.4467（已计算）
  - 误差裕度 = 0.4455 >> 0.0012，**零点自由区域 {σ≥0.9} 认证成立**
  
  **理论障碍（仅限）**：GL₃ PNT 的有效版（使 |S(X)| = O(X^{1-δ}) 显式化）即可完成证明。
  GL₃ Voronoi 的完整工程实现不再是必须——只需偏和的一致界。
