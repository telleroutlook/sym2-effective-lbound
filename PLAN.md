...
- **偏和实验扩展（2026-08-16，`discovery/_fast_tau_sieve.py`，N=1000000）**：
  ...
  **任意满足 C_GL3 ≤ Q^{1/3} 的 GL3 Voronoi 界均直接认证零点自由区域 {σ≥0.9}**。
  ...
  Voronoi 粗估计：C_GL3 ≈ Q^{1/6}×√C_RS×O(1) ≈ 6.32 < 7.488（若 O(1) 因子 ≤ 1）。
  对 σ=0.95，N=10^6：需 C < 6.93（更宽松，更容易满足）。
  经验常数约为 0.0037（N=10^7），比 N=10^8 阈值（7.49）小 2024 倍。
  实现 C_GL3 ≤ 6.93 的理论途径（任选其一）：
  (a) Miller-Schmid (2006) GL₃ Voronoi 显式常数（SL(3,Z) 导子1，谱参数 ν=(11/2, 0, -11/2)）——提取 O(Q^{1/3}) 已足够
